...
import { useWorkspaceStore, type ModuleId } from '@/stores/useWorkspaceStore';
import { useRouter, usePathname } from 'next/navigation';

const railItems: { id: ModuleId; icon: string; label: string; href: string }[] = [
  { id: 'workspace', icon: 'home', label: 'Workspace', href: '/' },
  { id: 'hr', icon: 'users', label: 'HR', href: '/hr' },
  { id: 'finance', icon: 'wallet', label: 'Finance', href: '/finance' },
  { id: 'inventory', icon: 'package', label: 'Inventory', href: '/inventory' },
  { id: 'procurement', icon: 'cart', label: 'Procurement', href: '/procurement' },
  { id: 'sales', icon: 'trend', label: 'Sales', href: '/sales' },
  { id: 'crm', icon: 'briefcase', label: 'CRM', href: '/crm' },
  { id: 'projects', icon: 'project', label: 'Projects', href: '/projects' },
  { id: 'timesheets', icon: 'clock', label: 'Timesheets', href: '/timesheets' },
  { id: 'expenses', icon: 'receipt', label: 'Expenses', href: '/expenses' },
  { id: 'chat', icon: 'message', label: 'Chat', href: '/conversations' },
  { id: 'omnichannel', icon: 'inbox', label: 'Omnichannel', href: '/conversations' },
  { id: 'helpdesk', icon: 'help', label: 'Helpdesk', href: '/conversations' },
  { id: 'documents', icon: 'file', label: 'Documents', href: '/documents' },
];
  ...
  return (
    <aside className="kc-rail">
      ...
      {railItems.map((item) => (
        <button
          key={item.id}
          className={`kc-rail-item${activeModule === item.id ? ' is-active' : ''}`}
          title={item.label}
          onClick={() => {
...
function RailIcon({ name }: { name: string }) {
  const icons: Record<string, React.ReactNode> = {
    ...
    wallet: <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><rect x="1" y="4" width="22" height="16" rx="2"/><path d="M1 10h22"/></svg>,
    package: <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M16.5 9.4l-9-5.19"/><path d="M21 16V8a2 2 0 0 0-1-1.73l-7-4a2 2 0 0 0-2 0l-7 4A2 2 0 0 0 3 8v8a2 2 0 0 0 1 1.73l7 4a2 2 0 0 0 2 0l7-4A2 2 0 0 0 21 16z"/><polyline points="3.27 6.96 12 12.01 20.73 6.96"/><line x1="12" y1="22.08" x2="12" y2="12"/></svg>,
    ...
    trend: <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><polyline points="23 6 13.5 15.5 8.5 10.5 1 18"/><polyline points="17 6 23 6 23 12"/></svg>,
    briefcase: <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><rect x="2" y="7" width="20" height="14" rx="2"/><path d="M16 21V5a2 2 0 0 0-2-2h-4a2 2 0 0 0-2 2v16"/></svg>,
    project: <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M2 3h6a4 4 0 0 1 4 4v14a3 3 0 0 0-3-3H2z"/><path d="M22 3h-6a4 4 0 0 0-4 4v14a3 3 0 0 1 3-3h7z"/></svg>,
    ...
    receipt: <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M4 2v20l2-1 2 1 2-1 2 1 2-1 2 1 2-1 2 1V2l-2 1-2-1-2 1-2-1-2 1-2-1-2 1-2-1z"/><path d="M8 10h8"/><path d="M8 14h4"/></svg>,
